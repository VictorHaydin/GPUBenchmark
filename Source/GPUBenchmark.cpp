...
void GPUBenchmark::run()
{
	...
	printf("***** Kernel latencies\n");
	printf("Host kernel schedule latency (1x1):               %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelScheduleTinyTask, 1, 1));
	printf("Host kernel schedule latency (1x32):              %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelScheduleTinyTask, 1, 32));
	printf("Host kernel schedule latency (8x64):              %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelScheduleTinyTask, 8, 64));
	printf("Host kernel schedule latency (16384x128):         %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelScheduleTinyTask, 16384, 128));
	printf("\n");
	printf("Host kernel execution latency (1x1):              %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelExecuteTinyTask, 1, 1));
	printf("Host kernel execution latency (1x32):             %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelExecuteTinyTask, 1, 32));
	printf("Host kernel execution latency (8x64):             %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelExecuteTinyTask, 8, 64));
	printf("Host kernel execution latency (16384x128):        %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelExecuteTinyTask, 16384, 128));
	printf("\n");

#ifdef CUDA50
	printf("***** Kernel latencies (dynamic parallelism)\n");
	printf("Device kernel schedule latency (1x1):             %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicScheduleTinyTask, 1, 1));
	printf("Device kernel schedule latency (1x32):            %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicScheduleTinyTask, 1, 32));
	printf("Device kernel schedule latency (8x64):            %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicScheduleTinyTask, 8, 64));
	printf("Device kernel schedule latency (16384x128):       %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicScheduleTinyTask, 16384, 128));
	printf("\n");
	printf("Device kernel execution latency (1x1):            %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicExecuteTinyTask, 1, 1));
	printf("Device kernel execution latency (1x32):           %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicExecuteTinyTask, 1, 32));
	printf("Device kernel execution latency (8x64):           %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicExecuteTinyTask, 8, 64));
	printf("Device kernel execution latency (16384x128):      %12.9f sec\n", execInnerTiming(&GPUBenchmark::kernelDynamicExecuteTinyTask, 16384, 128));
	printf("\n");
#endif

	printf("***** Reduction speed\n");
	printf("Reduction SUM (1x32,    256 elements, int):       %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<int>, 256, 10000, 1, 32));
	printf("Reduction SUM (1x32,    256 elements, float):     %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<float>, 256, 10000, 1, 32));
	printf("Reduction SUM (1x32,    256 elements, double):    %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<double>, 256, 10000, 1, 32));
	printf("Reduction SUM (128x64,  1M elements, int64):      %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<__int64>, 1048576, 10, 128, 64));
	printf("Reduction SUM (128x64,  1M elements, float):      %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<float>, 1048576, 10, 128, 64));
	printf("Reduction SUM (128x64,  1M elements, double):     %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<double>, 1048576, 10, 128, 64));
	printf("Reduction SUM (128x256, 1M elements, int64):      %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<__int64>, 1048576, 10, 128, 256));
	printf("Reduction SUM (128x256, 1M elements, float):      %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<float>, 1048576, 10, 128, 256));
	printf("Reduction SUM (128x256, 1M elements, double):     %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<double>, 1048576, 10, 128, 256));
	printf("Reduction SUM (512x512, 10M elements, int64):     %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<__int64>, 10485760, 1, 512, 512));
	printf("Reduction SUM (512x512, 10M elements, float):     %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<float>, 10485760, 1, 512, 512));
	printf("Reduction SUM (512x512, 10M elements, double):    %12.9f sec\n", execInnerTiming(&GPUBenchmark::reductionSum<double>, 10485760, 1, 512, 512));
	printf("\n");

	printf("***** Dependent FLOP\n");
	...
	printf("Device Memory allocate/release (16 bytes):        %12.9f sec\n", execOuterTiming(&GPUBenchmark::deviceMemAllocRelease, 16, 10) / 10);
	printf("Mapped Memory allocate/release (16 bytes):        %12.9f sec\n", execOuterTiming(&GPUBenchmark::mappedMemAllocRelease, 16, 10) / 10);
	printf("Host Memory register/unregister (16 bytes):       %12.9f sec\n", execOuterTiming(&GPUBenchmark::hostMemRegisterUnregister, 16, 10) / 10);
	printf("Host Write Combined allocate/release (16 bytes):  %12.9f sec\n", execOuterTiming(&GPUBenchmark::hostMemWriteCombinedAllocRelease, 16, 10) / 10);
	printf("\n");
	printf("Device Memory allocate/release (10M bytes):       %12.9f sec\n", execOuterTiming(&GPUBenchmark::deviceMemAllocRelease, 10485760, 1) / 1);
	printf("Mapped Memory allocate/release (10M bytes):       %12.9f sec\n", execOuterTiming(&GPUBenchmark::mappedMemAllocRelease, 10485760, 1) / 1);
	printf("Host Memory register/unregister (10M bytes):      %12.9f sec\n", execOuterTiming(&GPUBenchmark::hostMemRegisterUnregister, 10485760, 1) / 1);
	printf("Host Write Combined allocate/release (10M bytes): %12.9f sec\n", execOuterTiming(&GPUBenchmark::hostMemWriteCombinedAllocRelease, 10485760, 1) / 1);
	printf("\n");
	printf("Device Memory allocate/release (100M bytes):      %12.9f sec\n", execOuterTiming(&GPUBenchmark::deviceMemAllocRelease, 104857600, 1) / 1);
	printf("Mapped Memory allocate/release (100M bytes):      %12.9f sec\n", execOuterTiming(&GPUBenchmark::mappedMemAllocRelease, 104857600, 1) / 1);
	printf("Host Memory register/unregister (100M bytes):     %12.9f sec\n", execOuterTiming(&GPUBenchmark::hostMemRegisterUnregister, 104857600, 1) / 1);
	printf("Host Write Combined allocate/release (100M bytes):%12.9f sec\n", execOuterTiming(&GPUBenchmark::hostMemWriteCombinedAllocRelease, 104857600, 1) / 1);
	printf("\n");

	//printf("***** Memory transfer speed (10MB blocks)\n");
	//printf("Regular (Host to GPU):                            %12.3f GB/sec\n", (double)10485760.0 / execInnerTiming(&GPUBenchmark::memTransfer, 10485760, 0, 0) / 1024 / 1024 / 1024);
	//printf("Page locked (Host to GPU):                        %12.3f GB/sec\n", (double)10485760.0 / execInnerTiming(&GPUBenchmark::memTransfer, 10485760, 1, 0) / 1024 / 1024 / 1024);
	//printf("Write Combined (Host to GPU):                     %12.3f GB/sec\n", (double)10485760.0 / execInnerTiming(&GPUBenchmark::memTransfer, 10485760, 2, 0) / 1024 / 1024 / 1024);
	//printf("\n");
	//printf("Regular (GPU to Host):                            %12.3f GB/sec\n", (double)10485760.0 / execInnerTiming(&GPUBenchmark::memTransfer, 10485760, 0, 1) / 1024 / 1024 / 1024);
	//printf("Page locked (GPU to Host):                        %12.3f GB/sec\n", (double)10485760.0 / execInnerTiming(&GPUBenchmark::memTransfer, 10485760, 1, 1) / 1024 / 1024 / 1024);
	//printf("Write Combined (GPU to Host):                     %12.3f GB/sec\n", (double)10485760.0 / execInnerTiming(&GPUBenchmark::memTransfer, 10485760, 2, 1) / 1024 / 1024 / 1024);
	//printf("\n");
	//printf("Device (GPU to GPU):                              %12.3f GB/sec\n", (double)10485760.0 / execInnerTiming(&GPUBenchmark::memTransfer, 10485760, 0, 2) / 1024 / 1024 / 1024);
	//printf("\n");
